...
\begin{itemize}
  \item \textbf{Open questions}: what Edge computing cases involving autonomous
    multiple control planes can be expressed via one-way convergent (one-way
    convergence\cite{b2} greatly simplifies systems implementation) causal
    per-key consistency?
...
TODO alternatives: causal consistency per server (sticky?) and per replica(?)
