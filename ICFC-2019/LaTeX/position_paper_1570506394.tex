...
A. Lebre et al\cite{b9} bring vision of a unified Peer-to-Peer OpenStack IaaS
Manager, which is opposed to hierarchical distributed topologies that have
additional maintenance costs associated with it and require complex operations
in case of failures. That P2P IaaS Manager is positioned as an alternative
approach to federated clouds, which always involves brokerage middleware opaque
for the administrators/users because they cannot supervise the infrastructure
as they historically are used to. Such a view-point naturally complements the
vision of unified management plane as we introduced it for this paper. A. Lebre
et al used the Grid'5000 testbed for the proposed prototype validation purposes.
Experiments evaluated the use of Rome/Redis KVS as a replacement of
SQLAlchemy/MySQL for a single site case and distributed Galera cluster for
the multi-site deployment. The used data backends do not provide causal
consistency so there is some room remains for future experimenting perhaps.
