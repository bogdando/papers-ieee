...
logic and building middleware libraries on top of the underlying causal
consistent storage backends. That is, like Global causal consistent
databases\cite{b6}. The work presents potential applications and databases that
could use the causal consistency and shows possible methods to implement that
model. It also compares serializability, eventual and causal consistency using
a running example. There is an important conclusion that to the authors’
...
\begin{itemize}
  \item \textbf{Open questions}: does COPS work for multiple datacenters
    failure events, assuming at the edge there may be a lot of small failure
    domains? Does it cover bidirectional/two-way convergent causal+ replication?
  ...
  \item \textbf{Open questions}: any opensource implementations yet for causal
    consistent databases/KVS or midleware, like that RTC/COPS/Indigo etc.
    describes, to benefit OpenStack/Kubernetes cloud platforms for edge cases
    that rely on multiple autonomous control planes? If there is any, chances
    it can be integrated as a unified solution that fits all control/management
    planes cases for the platform, cloudlets and edge-native workloads? For
    example, consuming it as Replication-as-a-Service.
\end{itemize}
...
paper. The edge-cloud native APIs may be also designed with the approaches that
